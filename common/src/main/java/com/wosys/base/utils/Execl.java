...
public class Execl {
    ...
    static final int CELL_DATE = 4;
    static final int CELL_DATETIME = 5;
     ...
     * @param name  显示的名称
     */
    public void addColDate(String field, String name) {
        addCol(field, name, 25, CELL_DATE);
    }
     ...
     * @param name  显示的名称
     */
    public void addColDatetime(String field, String name) {
        addCol(field, name, 25, CELL_DATETIME);
    }
    ...
    public void addColSelect(String field, String name, String table, String value, String text, int mul) {
        ...
        Map selectCell = DB.name(table).field(value).field(text).getColkey(text, value);
            ...
            for (int row = 1; row < rows; row++) {
                ...
                for (int col = colStart; col < columns && col < fieldSize; col++) {
                    ...
                    if (type == CELL_SELECT) {
                        ...
                        field += "_text";
                    } else if (type == CELL_DATE || type == CELL_DATETIME) {
                        if (cellType == CellType.DATE) {
                            DateCell dc = (DateCell) cell;
                            Date date = dc.getDate();
                            TimeZone zone = TimeZone.getTimeZone("GMT");
                            SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd" + (type == CELL_DATETIME ? " HH:mm:ss" : ""));
