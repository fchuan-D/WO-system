...
import com.spcloud.app.provider.mapper.Type1Mapper;
...
public class Type1Service {
    ...
    @Resource
    private Type1Mapper mapper;

    /**
     * 根据Typename字段参数获取一行数据
     */
    public Type1 findByTypename(String username) {
        // 新建作业类型模块实体类Type1
        Type1 pojo = new Type1();
        // 设置参数
        pojo.setTypename(username);
        // 根据实体类新建QueryWrapper查询条件类
        QueryWrapper<Type1> queryWrapper = Wrappers.query(pojo);
        Type1 row = mapper.selectOne(queryWrapper);
        return row;
    }
     ...
     */
    public Type1 findByTypename(String username, Integer uid) {
        // 新建作业类型模块实体类Type1
        Type1 pojo = new Type1();
        // 设置参数
        pojo.setTypename(username);
     ...
     * 根据id 获取一行数据
     */
    public R<Type1> findById(Integer id) {
     ...
     * 根据Wrapper 对象进行数据筛选
     */
    public R<List<Type1>> selectAll(Wrapper<Type1> query) {
    ...
    public R<List<Type1>> selectAll() {
        QueryWrapper<Type1> wrapper = Wrappers.query();
    ...
    public R selectAll(Map<String, Object> map) {
        ...
        QueryWrapper<Type1> wrapper = mapToWrapper(map);
        // 设置排序
     ...
     */
    public QueryWrapper<Type1> mapToWrapper(Map<String, Object> map) {
        // 创建 QueryWrapper 对象
        QueryWrapper<Type1> wrapper = Wrappers.query();
    ...
    public R selectPages(QueryWrapper<Type1> wrapper, IPage page) {
    ...
    public R insert(Type1 entityData, Map post) {
        ...
        if (entityData.getId() != null) {
            ...
            DB.execute(
                    "INSERT INTO logs(module,operationtype,operationcontent,cx,username) values('作业类型' , '新增' , '类型名称：" +
                            post.get("typename") +
                            "' , '" +
    ...
    public R<Object> update(Type1 entityData, Map post) {
        if (StringUtil.isNullOrEmpty(entityData.getTypename())) {
            return R.error("请填写类型名称");
        }
        ...
        Integer charuid = entityData.getId();
        DB.execute(
                "INSERT INTO logs(module,operationtype,operationcontent,cx,username) values('作业类型' , '更新' , '类型名称：" +
                        post.get("typename") +
                        "' , '" +
                        SessionFactory.getCx() +
                        "' , '" +
    ...
    public R<Object> delete(Integer id) {
        try {
            Type1 map = mapper.selectById(id);
            Map post = DB.name("type1").find(id);

            mapper.deleteById(id);
            DB.execute(
                    "INSERT INTO logs(module,operationtype,operationcontent,cx,username) values('作业类型' , '删除' , '类型名称：" +
                            map.getTypename() +
                            "' , '" +
                            SessionFactory.getCx() +
                            "' , '" +
